...
class WaveDreamPipeline:
    ...
    async def _step_generate_structure(
        ...
        """Этап 3: Генерация структуры трека с автоматическим распределением энергии"""

        start_time = time.time()
        structure = None

        # Безопасные значения из request
        bpm_request = getattr(request, "custom_bpm", 120)
        key_request = getattr(request, "custom_key", "C")
        scale_request = getattr(request, "custom_scale", "major")
        custom_structure = getattr(request, "custom_structure", None)
        duration_request = getattr(request, "duration", 60)
        prompt_request = getattr(request, "prompt", "")

        # Используем переданный genre_info или пустой словарь
        genre_info = genre_info or getattr(request, "genre_info", {}) or {}

        def bars_to_seconds(bars: int, bpm: int) -> float:
            return bars * 4 * 60 / bpm

        def assign_energy(section_name: str) -> float:
            name = section_name.lower()
            if "intro" in name:
                return 0.2
            elif "verse" in name:
                return 0.5
            elif "chorus" in name or "drop" in name:
                return 0.8
            elif "bridge" in name:
                return 0.6
            elif "outro" in name:
        ...
        if custom_structure:
            ...
            sections = []
            current_time = 0
            for s in custom_structure:
                duration = bars_to_seconds(s.get("bars", 2), bpm)
                sections.append({
                    "type": s.get("name", "unknown").lower(),
                    "bars": s.get("bars", 2),
                    "duration": duration,
                    "start_time": current_time,
                    "energy": assign_energy(s.get("name", "unknown"))
                })
            ...
            self.logger.info(f"  🏗️ Используется кастомная структура: {len(sections)} секций")

        else:
            try:
                system_msg = """Ты музыкальный ассистент.
Верни только JSON без пояснений.
Формат строго:
{
  "BPM": число,
  "Key": "Am / C# / ...",
  "Scale": "minor или major",
  "Sections": [
    {"name": "Intro", "bars": 4},
    {"name": "Verse", "bars": 16},
    {"name": "Chorus", "bars": 8},
    {"name": "Outro", "bars": 4}
                ...
                response = ollama.chat(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": f"Сгенерируй структуру для трека: {prompt_request}"}
                    ]
                )
                raw_content = response.get('message', {}).get('content', "{}")
                llama_data = json.loads(raw_content)

                bpm = llama_data.get("BPM", 120)
                sections = []
                current_time = 0
                for s in llama_data.get("Sections", []):
                    duration = bars_to_seconds(s.get("bars", 2), bpm)
                    sections.append({
                        "type": s.get("name", "unknown").lower(),
                        "bars": s.get("bars", 2),
                ...
                structure = {
                    "BPM": bpm,
                    "Key": llama_data.get("Key", "C"),
                    "Scale": llama_data.get("Scale", "major"),
                    "sections": sections,
                    "total_duration": sum(s["duration"] for s in sections),
                    "source": "llama3-music"
                }
                self.logger.info(f"  🧠 Структура от LLaMA3: {len(sections)} секций")

            except Exception as e:
                self.logger.warning(f"  ⚠️ LLaMA3 недоступна или JSON невалиден: {e}")
                structure = self._generate_fallback_structure(genre_info, duration_request)
                for s in structure.get("sections", []):
                    s["energy"] = assign_energy(s.get("type", "unknown"))
        ...
        try:
            ...
            structure = {
                "sections": [
                    {"type": "intro", "duration": 8, "energy": 0.2, "start_time": 0},
                    {"type": "main", "duration": 32, "energy": 0.7, "start_time": 8},
                    {"type": "outro", "duration": 8, "energy": 0.3, "start_time": 40}
                ],
                "total_duration": 48,
        ...
        self.logger.info(f"  ⏱️ Общая длительность: {structure['total_duration']:.2f}с")
        self.logger.info(f"  ⏱️ Время обработки: {processing_time:.2f}с")
